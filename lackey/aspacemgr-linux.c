...
static void lk_parse_procselfmaps (Addr value,
...
void read_procselfmaps ( void )
{
   Int    n_chunk;
   SysRes fd;
   Int buf_n = 0;
   Long m_procselfmap_buf = 100000;
   HChar procselfmap_buf[m_procselfmap_buf];
   ...
   //buf_n_tot = 0;
   do {
      n_chunk = ML_(am_read)( sr_Res(fd), &procselfmap_buf[buf_n],
                              m_procselfmap_buf - buf_n );
      if (n_chunk >= 0)
         buf_n += n_chunk;
   } while ( n_chunk > 0 && buf_n < m_procselfmap_buf );

   ML_(am_close)(sr_Res(fd));

   if (buf_n >= m_procselfmap_buf-5)
      ML_(am_barf_toolow)("m_procselfmap_buf");
   if (buf_n == 0)
      ML_(am_barf)("I/O error on /proc/self/maps");

   procselfmap_buf[buf_n] = 0;
   VG_(printf)("%s\n", procselfmap_buf);
   //return procselfmap_buf;
}
   ...
   while (True) {
    ...
    read_line_ok:
	 ...
	 tmp = 0;
      ...
      if (record_mapping && start < endPlusOne)
         (*record_mapping) ( value, start, endPlusOne-start,
                             prot, dev, ino,
                             foffset, filename );
   ...
   { const Addr commpage_start = ARM_LINUX_FAKE_COMMPAGE_START;
     ...
     if (gapStart < commpage_start) {
        if (record_gap)
           (*record_gap)( gapStart, commpage_start - gapStart );
        if (record_mapping)
           (*record_mapping)( value, commpage_start, commpage_end1 - commpage_start,
                              VKI_PROT_READ|VKI_PROT_EXEC,
                              0/*dev*/, 0/*ino*/, 0/*foffset*/,
                              NULL);
        gapStart = commpage_end1;
   ...
   )
{
   ...
   while (1) {
      ...
      if (record_mapping) {
         (*record_mapping)(value, addr, size, mach2vki(info.protection),
...
/*my version of parse procselfmaps with addr position detector*/
/*static Bool lk_parse_procselfmaps (Addr value,
      Bool (*record_mapping)( Addr value, Addr addr, SizeT len, UInt prot,
                              ULong dev, ULong ino, Off64T offset,
                              const HChar* filename ),
      void (*record_gap)( Addr addr, SizeT len )
   )
{
   ...
   while (1) {
      ...
      if (record_mapping) {
         Bool rezult = (*record_mapping)(value, addr, size, mach2vki(info.protection),
                           0, 0, info.offset, NULL);
         if (rezult)
             return True;
      }
